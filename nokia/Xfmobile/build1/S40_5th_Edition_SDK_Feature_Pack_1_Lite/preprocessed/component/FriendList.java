...
public class FriendList extends ComponentUI {
    ...
    public synchronized boolean setFriendStatus(String email, int state) {
     ...
     public synchronized boolean setFriendStatus(int  userid, int state,String smsg) {
        for(int i=0; i<friends.size(); i++) {
            Friend f = (Friend)friends.elementAt(i);
            if(f.userid==userid) {
               // if(f.getState()!=state) {
                    if (smsg!=null ){
                        ...
                        System.out.println("update name "+f.getName()+"["+state+"]");

                        if (state!=3) {

                            f.setDisplayName(f.getOrignalName()+"  "+smsg);
                        }
                    }
                    f.setState(state);
