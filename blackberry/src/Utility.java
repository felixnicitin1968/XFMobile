...
public class Utility {
	...
	public static int m_connectiontype = 1; 
	
	public static int CONN_PREFER_WIFI = 1; 
	public static int CONN_WIFI_ONLY = 2; 
	public static int CONN_RADIO_ONLY = 3; 
	public static int CONN_MODE_WIFI = 4; 
	public static int CONN_MODE_RADIO = 5; 
	public static int CONN_MODE_NONE = 6; 
	public static String STR_EMPTY = ""; 
	public static int m_currentConnection = 6; 
	public static String m_radioAPN = STR_EMPTY; 
	public static String m_radioWAP2;
	public static String m_radioCarrier; 
	public static String m_userAPN = null; 
	...
	{ 
		...
		m_radioCarrier = RadioInfo.getCurrentNetworkName();
		if( m_radioCarrier == null ) m_radioCarrier = STR_EMPTY; 
		boolean radio = false; 
		try
		{ 
			radio = CoverageInfo.isCoverageSufficient(CoverageInfo.COVERAGE_DIRECT); 
		...
		if( m_connectiontype != CONN_WIFI_ONLY || 
				( m_connectiontype == CONN_PREFER_WIFI && !wifi ) ) 
		{
			if( m_radioCarrier.length() == 0 && RadioInfo.getCurrentNetworkName() != null 
					 && m_userAPN == null && radio ) 
			{
				...
				if( m_radioWAP2 == null )
				{
					m_radioWAP2 = getWAP2ServiceBook(); 
				}
				try
				{ 
		     ...
		     if( m_connectiontype == CONN_PREFER_WIFI ) 
		     {
		    	 if( m_currentConnection == CONN_MODE_NONE ) 
		    	 {
		    		 if( wifi ) 
		    		 
		    		 {
		    			 m_currentConnection = CONN_MODE_WIFI; 
		    			 return STR_WIFI; 
		    			 } 
		    		 else 
		    		 { 
		    			 m_currentConnection = CONN_MODE_RADIO; 
		    			 return m_radioAPN;
		    			 } 
		    		 } 
		    	 if( wifi )
		     ...
		     if( m_connectiontype == CONN_RADIO_ONLY )
		     {
		    	 m_currentConnection = CONN_MODE_RADIO; return m_radioAPN; 
		     }
		     
		     if( radio ) m_currentConnection = CONN_MODE_RADIO; return STR_EMPTY;
		     
	}
	
	  public static String getWAP2ServiceBook() 
      {
     	 if( m_userAPN != null ) return m_userAPN;
     	 String connstring = null; 
     	 ServiceBook book = ServiceBook.getSB(); 
     	 ServiceRecord[] records = book.findRecordsByCid("WPTCP"); 
     	 for( int i = 0; i < records.length; i++ )
     	 { 
     		 //System.err.println("ServiceRecord -- " + records[i].getUid() + " -- " + records[i].getName() + " -- " + records[i].getId() + " -- " + records[i].getCid());
     		 if( records[i].isValid() && !records[i].isDisabled() && records[i].getUid() != null && records[i].getUid().length() != 0 && records[i].getUid().toLowerCase().indexOf("wifi") == -1 )
     		 
     		 {
     			 connstring = STR_WAP2 + records[i].getUid();
	...
	static String byteArrayToHexString(byte in[], int length)
	 ...
	 static String getConnectionString()
	{
		...
		if ((CoverageInfo.getCoverageStatus() & net.rim.device.api.system.CoverageInfo.COVERAGE_BIS_B) == CoverageInfo.COVERAGE_BIS_B)
		{
			...
			{
				st = ";deviceside=false;connectionUID=" + rec.getUid() + ";ConnectionType=mds-public";
			}
		}
	...
	/*
	// String url =
	// "http://10.0.0.1;WAPGatewayIP=127.0.0.1;WAPGatewayAPN=carrier.com.gprs";
	static String getConnectionString() {
		...
		// In theory, all bases are covered so this shouldn't be reachable.
		 ...
		 return connectionString + ";ConnectionTimeout=20000";
...
*/
	...
	static String getGameName(int gameid) {
