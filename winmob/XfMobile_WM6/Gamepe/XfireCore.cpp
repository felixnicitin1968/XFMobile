...
void CXfireCore::Invite(unsigned char *sid)
{
	...
	SendIM(sid,imindex,"invite to join Xfire mobile @ http://www.xf-mobile.com");
...
void CXfireCore::HandleStatus(unsigned char *pBuffer ,unsigned short dwMsgSize)
{
	...
	unsigned int  users_id_buffersize=read_attribute_noname(pBuffer,ptr,ARRAY_TYPE,0);
	...
	char   *users_id_ptr=(char*)malloc(users_id_buffersize);
	
	ptr=read_attribute_noname(pBuffer,ptr,ARRAY_TYPE,(uint8*)users_id_ptr);
	
	
	
	unsigned int  users_sid_buffersize=read_attribute_noname(pBuffer,ptr,3,0);
...
void CXfireCore::ParseMessage(unsigned char *pBuffer  ,USHORT dwMsgSize)
{
	...
	OutputDebugString(L"ParseMessage\n");

	DbgMsg(L"msg in %d",cPacketID);
	switch(cPacketID){
	...
	//	CApp::PutLogList( "chat room accessibility change received\n");
		...
		case XFIRE_PACKET_CONTACTS:
			...
			OutputDebugString(L"XFIRE_PACKET_CONTACTS\n");
		...
		if (nRead > 0) 
		{
			...
			if (m_nBytesRecv == m_nRecvDataLen) 
			{
				...
				if (m_bReadDataLength) 
				{
					...
					m_nBytesRecv = 0;
					//OutputDebugString(_TEXT("CConnectSoc::OnReceive Header length = %d\n"), m_nRecvDataLen);
				} 
				else 
				{
