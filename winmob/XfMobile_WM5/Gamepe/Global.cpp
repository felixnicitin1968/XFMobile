...
WCHAR            g_wzDeviceUDID[0x100]=L"00000000000000000000000000000000";
	...
	while(1)
	{
		...
		if (HttpQueryInfo(hRequest, HTTP_QUERY_STATUS_CODE  ,
			lpvSomeBuffer, &dwSize, NULL)){
				...
				DbgMsg(L"SendURLRequest result ....\n");
...
UINT __cdecl MetricImpressionsNotificatioThread( LPVOID pParam )
{
...
BOOL   DownloadImage(HWND hWnd,WCHAR *szURL)
{
    ...
    DbgMsg(L"*********DownloadImage*****************\n");

	
	UINT uRetVal;
	DWORD dwBufSize=MAX_PATH;
	WCHAR lpPathBuffer[MAX_PATH]=L"\\My Documents\\XFMOBILE\\xfmobilead";
	...
	if (sOK==S_OK){
		...
		return TRUE;
		  ...
		  int x=GetLastError();
		  DbgMsg(L"*********URLDownloadToFile FAILED***[%08X]**************\n",x);
...
BOOL DownloadXMLFromADGateway(LPWSTR pServer,LPWSTR pObject)
{
	...
	{ 
		...
		DbgMsg (L"CreateFile failed with error %08X.\n", GetLastError());
